...
CREATE TABLE zc_stage.stg_patient (
    ...
    raw_payload CLOB, record_hash VARCHAR2(64) NOT NULL,
    ...
    CONSTRAINT ck_stg_patient_json CHECK (raw_payload IS JSON),
...
CREATE TABLE zc_stage.stg_practitioner (
    ...
    raw_payload CLOB, record_hash VARCHAR2(64) NOT NULL,
    validation_status VARCHAR2(20) DEFAULT 'PENDING' NOT NULL,
    validation_message VARCHAR2(1000), staged_at TIMESTAMP WITH TIME ZONE DEFAULT SYSTIMESTAMP NOT NULL,
    CONSTRAINT pk_stg_practitioner PRIMARY KEY (batch_id, source_system, source_record_id),
    CONSTRAINT ck_stg_practitioner_json CHECK (raw_payload IS JSON),
...
CREATE TABLE zc_stage.stg_facility (
    ...
    CONSTRAINT ck_stg_facility_json CHECK (raw_payload IS JSON),
...
CREATE TABLE zc_stage.stg_encounter (
    ...
    CONSTRAINT ck_stg_encounter_json CHECK (raw_payload IS JSON),
...
CREATE TABLE zc_stage.stg_condition (
    ...
    CONSTRAINT ck_stg_condition_json CHECK (raw_payload IS JSON),
...
CREATE TABLE zc_stage.stg_observation (
    ...
    CONSTRAINT ck_stg_observation_json CHECK (raw_payload IS JSON),
